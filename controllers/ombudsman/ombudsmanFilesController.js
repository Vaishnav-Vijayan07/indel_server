  ...
  static async getAll(req, res, next) {
    try {
      ...
      if (!page && !limit) {
        ...
        return res.json({ success: true, data: ombudsmanFiles });
      }
      ...
      const cacheKey = search ? null : `ombudsmanFiles_page_${pageNum}_limit_${limitNum}`;
      if (cacheKey) {
        const cachedData = await CacheService.get(cacheKey);
        if (cachedData) {
      ...
      const { count, rows } = await OmbudsmanFiles.findAndCountAll({
